...
coverage:
	coverage run --source=deconvolution -m unittest discover -s tests > /dev/null; coverage report

# Check test coverage and show the results in browser
html:
	coverage run --source=deconvolution -m unittest discover -s tests > /dev/null; coverage html; python -m webbrowser "./htmlcov/index.html" &
